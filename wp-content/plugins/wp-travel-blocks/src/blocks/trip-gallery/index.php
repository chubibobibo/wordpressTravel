...
function wptravel_blocks_trip_gallery_block_callback( $attributes ) {
	if ( is_admin() ) {
		return '<div></div>';
	}
	ob_start();
	...
	echo '<div id="gallery-blocks" class="wptravel-block-wrapper wptravel-block-trip-gallery ' . esc_attr( $align_class ) . '">'; //@phpcs:ignore
 ...
 */
function wptravel_blocks_register_trip_gallery_block() {
	// Return early if this function does not exist.
	if ( ! function_exists( 'register_block_type' ) ) {
		return;
	}
	ob_start();
	include WPTRAVEL_BLOCKS_PLUGIN_DIR . 'src/blocks/trip-gallery/block.json';
	$metadata   = json_decode( ob_get_clean(), true );
	$attributes = $metadata['attributes'];
	register_block_type(
		'wptravel/trip-gallery',
		array(
			'attributes'      => $attributes,
			'editor_script'   => 'wptravel-blocks-editor',
			'editor_style'    => 'wptravel-blocks-editor',
			'style'           => 'wptravel-blocks-frontend',
			'render_callback' => 'wptravel_blocks_trip_gallery_block_callback',
		)
	);
}
add_action( 'init', 'wptravel_blocks_register_trip_gallery_block' );
