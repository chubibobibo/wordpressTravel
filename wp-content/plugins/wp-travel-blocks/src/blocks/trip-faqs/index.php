...
function wptravel_blocks_trip_faqs_block_callback( $attributes ) {
	if ( is_admin() ) {
		return '<div></div>';
	}
	ob_start();
	...
	echo '<div id="faq-blocks" class="wptravel-block-wrapper wptravel-block-trip-faqs ' . esc_attr( $align_class ) . '">'; //@phpcs:ignore
 ...
 */
function wptravel_blocks_register_trip_faqs_block() {
	// Return early if this function does not exist.
	if ( ! function_exists( 'register_block_type' ) ) {
		return;
	}
	ob_start();
	include WPTRAVEL_BLOCKS_PLUGIN_DIR . 'src/blocks/trip-faqs/block.json';
	...
	register_block_type(
		...
		array(
			...
			'render_callback' => 'wptravel_blocks_trip_faqs_block_callback',
		)
	);
}
add_action( 'init', 'wptravel_blocks_register_trip_faqs_block' );
