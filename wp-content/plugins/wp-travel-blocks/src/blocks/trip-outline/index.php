...
function wptravel_blocks_trip_outline_block_callback( $attributes ) {
	if ( is_admin() ) {
		return '<div></div>';
	}
	ob_start();
	...
	echo '<div class="wptravel-block-wrapper wptravel-block-trip-outline ' . esc_attr( $align_class ) . '">'; //@phpcs:ignore
 ...
 */
function wptravel_blocks_register_trip_outline_block() {
	...
	register_block_type(
		'wptravel/trip-outline',
		array(
			'attributes'      => $attributes,
			'editor_script'   => 'wptravel-blocks-editor',
			'editor_style'    => 'wptravel-blocks-editor',
			'style'           => 'wptravel-blocks-frontend',
			'render_callback' => 'wptravel_blocks_trip_outline_block_callback',
		)
	);
}
add_action( 'init', 'wptravel_blocks_register_trip_outline_block' );
